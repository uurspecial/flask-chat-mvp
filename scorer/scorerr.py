...
if __name__ == '__main__':
    ...
    csv_file = "results_Llama-Breeze2-8B-Instruct_zero-shot.csv"
    print(f"📥 正在載入檔案: {csv_file}")
    df = pd.read_csv(csv_file)
    
    new_scores = []
    is_correct_list = []
    
    print(f"🚀 開始評分，共 {len(df)} 筆資料...")
    
    for index, row in df.iterrows():
        text_to_score = str(row['generated_text'])
        target_level = int(row['target_level'])
        
        predicted_score = scorer.score(text_to_score)
        new_scores.append(predicted_score)
        
        is_correct = (predicted_score == target_level)
        is_correct_list.append(is_correct)
        
        print(f"[{index+1}/{len(df)}] 目標: {target_level} | 預測: {predicted_score} | {'✅' if is_correct else '❌'} | 句子: {text_to_score}")
        time.sleep(0.1) 
        
    df['predicted_score'] = new_scores
    df['is_correct'] = is_correct_list
    
    output_file = "rescored_results.csv"
    df.to_csv(output_file, index=False, encoding="utf-8-sig")
    print(f"\n✅ 評分完成！結果已儲存至: {output_file}")
    
    total_correct = df['is_correct'].sum()
    total_count = len(df)
    overall_accuracy = (total_correct / total_count) * 100
    print(f"\n📊 整體準確率: {total_correct} / {total_count} ({overall_accuracy:.2f}%)")
    
    print("\n📈 各等級準確率統計:")
    stats = df.groupby("target_level")["is_correct"].agg(['sum', 'count', 'mean'])
    for level, row in stats.iterrows():
        correct_num = int(row['sum'])
        level_total = int(row['count'])
        accuracy_pct = row['mean'] * 100
        print(f"  ▶ Level {level}: 正確 {correct_num} 句 / 共 {level_total} 句 ({accuracy_pct:.2f}%)")